...
def input_parser(v, errors, extra) -> any:
    ...
    errors_text = f"\t[⚡] Errors: {errors}" if errors != "None." else ""
    extra_text = f"\t[🔍] Answer: {extra}" if extra != "None." else ""
    ...
    if operation_number == 0:
        ...
        if len(operation_args) == 3:
            old_values = [x.strip() for x in operation_args[1].replace('[', '').replace(']', '').split(',')]
            new_values = [x.strip() for x in operation_args[2].replace('[', '').replace(']', '').split(',')]
...
def update_menu():
    ...
    """
    current_list, last, errors, extra = [], [], "None.", "None."
    save_list(current_list, last)
    while True:
        os.system('cls||clear')
        try:
            user_input = input_parser(current_list, errors, extra)
            errors = "None."
        ...
        try:
            ...
            # <------------> GET PRIMES, ODDS, SUM, GCD, MAX <------------>
                ...
                extra = {3: get.prime, 4: get.odd, 5: prop.sum, 6: prop.gcd, 7: prop.max}[operation_number](current_list, start_index, stop_index)
            # <------------> REMOVE NON PRIMES <------------>
